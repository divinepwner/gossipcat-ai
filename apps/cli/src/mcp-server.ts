...
const MCP_TOOLS = [
    ...
    inputSchema: {
      ...
      properties: {
        task: {
          type: 'string',
          description:
            'The task to execute. Be specific about what you want and which files are relevant.',
        },
        strategy: {
          type: 'string',
          enum: ['auto', 'parallel', 'sequential'],
          description:
            'Execution strategy. "auto" lets the orchestrator decide. Default: auto.',
        },
      },
      required: ['task'],
    },
  },
  {
    name: 'gossip_agents',
    description:
      'List all configured agents in the Gossip Mesh team, including their provider, model, role, and skills. Use this to understand what agents are available before orchestrating.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    name: 'gossip_status',
    description:
      'Check the status of the Gossip Mesh system — relay, tool server, and connected agents.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
...
class GossipMcpServer {
        ...
        try {
          ...
          return this.respond(request.id, {
            content: [
              {
                type: 'text',
                text: `Orchestration error: ${(err as Error).message}`,
              },
            ],
          });
        }
  ...
  private async boot(): Promise<void> {
    ...
    if (!configPath) {
      throw new Error(
        'No gossip.agents.json found. Run gossipcat setup first.'
      );
    }
    ...
    this.mainAgent = new MainAgent(mainAgentConfig);
    await this.mainAgent.start();
    this.initialized = true;

    // Log to stderr — stdout is reserved for MCP JSON-RPC
    process.stderr.write(
      `[gossipcat-mcp] Booted: relay :${this.relay.port}, ${agentConfigs.length} agents\n`
    );
  }
  ...
  process.stdin.on('data', (chunk: Buffer) => {
    ...
    while (true) {
      ...
      if (headerEnd === -1) {
        ...
        if (line) {
          handleLine(server, line).catch((err: Error) => {
            process.stderr.write(
              `[gossipcat-mcp] Unhandled error: ${err.message}\n`
            );
          });
        }
        continue;
      ...
      handleLine(server, body).catch((err: Error) => {
        process.stderr.write(
          `[gossipcat-mcp] Unhandled error: ${err.message}\n`
        );
      });
...
async function handleLine(
  ...
  try {
    ...
    process.stderr.write(
      `[gossipcat-mcp] Parse error: ${(err as Error).message}\n`
    );
  }
}
