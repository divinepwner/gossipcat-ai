...
async function runSetupWizard() {
        ...
        const key = await p.password({
            ...
            validate: (val) => {
                if (!val || val.trim().length === 0)
                    return 'API key is required. Deselect the provider to skip it.';
            },
        });
        if (p.isCancel(key)) {
            p.cancel('Setup cancelled.');
    ...
    const mainModelOptions = mainProvider === 'local'
        ...
        : PROVIDERS[mainProvider].models.map(m => ({
    ...
    const summary = Object.entries(agents)
        .map(([id, a]) => `  ${id} → ${a.model} (${a.preset})`)
        .join('\n');
    p.note(`Orchestrator: ${mainModel} (${mainProvider})\n\nTeam (${agentCount} agent${agentCount > 1 ? 's' : ''}):\n${summary}`, 'Your Setup');
