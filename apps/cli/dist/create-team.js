...
// ── Main command ────────────────────────────────────────────────────────────
async function createTeam(description) {
    ...
    if (!description) {
        const input = await p.text({
            ...
            validate: (v) => { if (!v?.trim() || v.trim().length < 10)
                return 'Tell me more about your project and needs (at least 10 characters)'; },
        });
        if (p.isCancel(input)) {
            p.cancel('Cancelled.');
            process.exit(0);
    ...
    let teamConfig;
    try {
        llm = (0, orchestrator_1.createProvider)(plannerProvider, plannerModel, plannerKey || undefined);
...
Propose the optimal agent team for this project.`;
        ...
        s.stop('Failed');
        p.log.error(`Failed to generate team: ${err.message}`);
    ...
    if (action === 'modify') {
        ...
        try {
            ...
            s2.stop('Failed');
            p.log.error(`Modification failed: ${err.message}`);
        ...
        if (agent.provider !== 'local' && !available.includes(agent.provider)) {
            ...
            const key = await p.password({
                ...
                validate: (v) => { if (!v?.trim())
                    return 'Required for this agent'; },
            });
            if (p.isCancel(key)) {
                p.cancel('Cancelled.');
                process.exit(0);
    ...
    for (const agent of teamConfig.agents) {
        ...
        (0, create_agent_1.createAgentDirectory)(agent.id, agentConfig);
    }
    // Write config
