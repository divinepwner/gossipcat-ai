...
const tools_2 = require("@gossip/tools");
...
describe('E2E: Full Gossip Mesh Flow', () => {
    ...
    beforeAll(async () => {
        ...
        testProjectDir = (0, path_1.resolve)((0, os_1.tmpdir)(), `gossip-e2e-test-${Date.now()}`);
        (0, fs_1.mkdirSync)((0, path_1.resolve)(testProjectDir, 'src'), { recursive: true });
        (0, fs_1.writeFileSync)((0, path_1.resolve)(testProjectDir, 'src/example.ts'), 'export function hello(): string {\n  return "Hello from Gossip Mesh!";\n}\n');
        // Start relay on a random port
        relay = new relay_1.RelayServer({ port: 0 });
        await relay.start();
        // Start tool server pointed at the test project
        toolServer = new tools_1.ToolServer({
            relayUrl: relay.url,
    ...
    afterAll(async () => {
        ...
        await relay.stop();
        try {
            (0, fs_1.rmSync)(testProjectDir, { recursive: true, force: true });
        }
        catch { /* ignore */ }
    });
    ...
    it('routes a task through the full pipeline: worker → tool-server → result', async () => {
        ...
        const worker = new orchestrator_1.WorkerAgent('test-worker', mockLLM, relay.url, tools_2.ALL_TOOLS);
        await worker.start();
    ...
    it('tool server denies access to paths outside project root', async () => {
        ...
        const worker = new orchestrator_1.WorkerAgent('evil-worker', evilLLM, relay.url, tools_2.ALL_TOOLS);
        await worker.start();
        await new Promise(r => setTimeout(r, 100));
        // The tool-server rejects the path; the worker propagates it as a thrown error
        await expect(worker.executeTask('Read /etc/passwd')).rejects.toThrow(/outside project root/i);
        await worker.stop();
    }, 30_000);
    ...
    it('handles tool call timeout gracefully when no tool server is registered', async () => {
        ...
        const isolatedRelay = new relay_1.RelayServer({ port: 0 });
        ...
        const mockLLM = new MockLLM();
        const worker = new orchestrator_1.WorkerAgent('timeout-worker', mockLLM, isolatedRelay.url, tools_2.ALL_TOOLS);
        await worker.start();
    ...
    it('multiple agents can communicate through the relay simultaneously', async () => {
        const agentA = new client_1.GossipAgent({ agentId: 'e2e-agent-a', relayUrl: relay.url });
        const agentB = new client_1.GossipAgent({ agentId: 'e2e-agent-b', relayUrl: relay.url });
        const agentC = new client_1.GossipAgent({ agentId: 'e2e-agent-c', relayUrl: relay.url });
        ...
        const receivedByB = [];
        const receivedByC = [];
        agentB.on('message', (data) => receivedByB.push(data));
        agentC.on('message', (data) => receivedByC.push(data));
